...
async fn check_master(claims: &Claims, state: &AppState) -> Result<AdminRecord, (StatusCode, Json<ErrorResponse>)> {
    ...
    match admin {
        ...
        _ => Err((StatusCode::FORBIDDEN, Json(ErrorResponse { error: "Acesso negado. Apenas masters.".to_string() })))
    }
}
...
pub async fn get_admins(
    ...
    let admins = sqlx::query_as!(
        AdminListItem,
        "SELECT id, name, email, phone, is_active as \"is_active: bool\", is_master as \"is_master: bool\" FROM admins ORDER BY id"
    )
...
pub async fn create_admin(
    ...
    if payload.is_master && req_admin.email != "master@master.master" {
        return Err((StatusCode::FORBIDDEN, Json(ErrorResponse { error: "Apenas master@master.master pode criar novos masters.".to_string() })));
    }
    ...
    if res.is_err() {
        return Err((StatusCode::BAD_REQUEST, Json(ErrorResponse { error: "Erro ao criar admin. Email já existe?".to_string() })));
    }
...
pub async fn update_admin_status(
    ...
    if id == 1 && req_admin.id != 1 {
        return Err((StatusCode::FORBIDDEN, Json(ErrorResponse { error: "Não pode alterar o master principal.".to_string() })));
    }
    ...
    if target_admin.is_none() {
        return Err((StatusCode::NOT_FOUND, Json(ErrorResponse { error: "Admin não encontrado".to_string() })));
    }
    ...
    if let Some(is_master) = payload.is_master {
        if is_master != target_is_master && req_admin.email != "master@master.master" {
            return Err((StatusCode::FORBIDDEN, Json(ErrorResponse { error: "Apenas master@master.master pode alterar nível master.".to_string() })));
        }
    ...
    if let Some(is_active) = payload.is_active {
        if id == 1 && !is_active {
            return Err((StatusCode::FORBIDDEN, Json(ErrorResponse { error: "Não pode desativar o master principal.".to_string() })));
        }
...
pub async fn get_system_logs(
    ...
    if filters.log_type.is_none() || filters.log_type.as_deref() == Some("login") {
        let mut query = String::from("SELECT id, \"login\" as log_type, severity, email as description, NULL as admin_id, NULL as animal_id, remote_ip, timestamp FROM login_logs WHERE 1=1");
        if let Some(ref sev) = filters.severity { query.push_str(&format!(" AND severity = \"{}\"", sev)); }
        ...
        for r in res { logs.push(SystemLog { id: r.id, log_type: r.log_type, severity: r.severity, description: format!("Tentativa de login: {}", r.description), admin_id: r.admin_id, animal_id: r.animal_id, remote_ip: r.remote_ip, timestamp: r.timestamp }); }
    }
